...
async function migrateData() {
  ...
  try {
    ...
    for (const table of tables) {
        ...
        if (!exists) {
          ...
          try {
            ...
            try {
              ...
              // attempt to create indexes defined in sqlite for this table
              try {
                const idxs = await sqliteAll(`SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name=$1 AND sql NOT NULL`, [tableName]);
                for (const idx of idxs) {
                  let idxSql = idx.sql;
                  // SQLite-specific tokens: remove AUTOINCREMENT and unsupported "IF NOT EXISTS" in index SQL
                  idxSql = idxSql.replace(/AUTOINCREMENT/ig, '');
                  idxSql = idxSql.replace(/IF NOT EXISTS/ig, '');
                  try {
                    await pgClient.query(idxSql);
                    logger.info({ index: idx.name, table: tableName }, `Created index ${idx.name} on ${tableName}`);
                  } catch (e) {
                    logger.warn({ err: e, index: idx.name, sql: idxSql }, `Could not create index ${idx.name} on ${tableName} (best-effort)`);
                  }
                }
              } catch (e) {
                logger.warn({ e, table: tableName }, 'Failed to enumerate/create sqlite indexes for table');
              }
          } else {
      ...
      // detect pk columns for ON CONFLICT handling
      let pkCols = [];
      try {
        const info = await sqliteAll(`PRAGMA table_info(${tableName})`);
        pkCols = (info || []).filter(c => c && c.pk).map(c => c.name);
      } catch (e) {
        logger.warn({ e, table: tableName }, 'Could not determine PK columns for conflict handling');
      }
      ...
      if (!RECONCILE_ONLY) {
        for (const row of rows) {
          ...
          const queryBase = `INSERT INTO ${tableName} (${columns.map(c => `"${c}"`).join(', ')}) VALUES (${placeholders})`;
          const conflictClause = (pkCols && pkCols.length > 0) ? ` ON CONFLICT (${pkCols.map(c => `"${c}"`).join(',')}) DO NOTHING` : '';
          const query = queryBase + conflictClause;
